    ...
    if (
      ...
      try {
        ...
        if (!email || !mobile || !Number.isFinite(amount) || amount <= 0) {
          return json(
            { error: "Please provide a valid email, mobile number and amount." },
            400
          );
        }
        ...
        try {
          ...
          return json(
            { error: "Rapid Gateway returned an invalid token response." },
            502
          );
        }
        ...
        if (!accessToken) {
          return json(
            { error: "No access token was returned by Rapid Gateway." },
            502
          );
        }
        ...
        try {
          ...
          return json(
            { error: "Rapid Gateway returned an invalid checkout response." },
            502
          );
        }
